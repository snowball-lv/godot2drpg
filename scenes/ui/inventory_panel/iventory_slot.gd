...
var slota_data: SlotData

func load_data(data: SlotData) -> void:
	slota_data = data
	if slota_data and slota_data.item:
		item_icon.texture = slota_data.item.icon
		item_icon.show()
		if slota_data.quantity > 1:
			amount_label.text = str(slota_data.quantity)
			amount_label.show()
		else:
			amount_label.hide()
	else:
